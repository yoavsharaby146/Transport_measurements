...
matplotlib.use("TkAgg")  # FIX: Force Matplotlib to use Tkinter backend immediately
...
class InteractivePlotter:
    def __init__(self, root):
        self.root = root
        self.root.title("Interactive CSV Plotter (Complete)")

        # --- DYNAMIC WINDOW SIZING (ROBUST) ---
        try:
            # Get user's screen dimensions
            screen_width = self.root.winfo_screenwidth()
            screen_height = self.root.winfo_screenheight()

            # Use 85% of the screen size (safer for laptops)
            w = int(screen_width * 0.85)
            h = int(screen_height * 0.85)

            # Ensure it never exceeds explicit maximums, but NO hard minimums that break small screens
            w = min(1400, w)
            h = min(980, h)
            ...
            print(f"Warning: Could not auto-size window ({e}). Using default.")
        ...
        # --- PERSISTENT VARIABLES (State Management) ---
        # Data Transform
        ...
        self.v_z_min = tk.StringVar()  # NEW: Z Range Min
        self.v_z_max = tk.StringVar()  # NEW: Z Range Max
        ...
        self.sci_x = tk.BooleanVar(value=True)
        self.sci_y = tk.BooleanVar(value=True)
    ...
    def setup_ui(self):
        ...
        ttk.Button(control_frame, text="Load CSV File(s)", command=self.load_files).grid(row=row, column=0,
                                                                                         ...
                                                                                         pady=5)
        row += 1
        ttk.Label(control_frame, text="Select Data (Hold Ctrl/Shift):").grid(row=row, column=0, columnspan=4,
                                                                             sticky='w')
        row += 1
        ...
        ttk.Label(self.y_dual_frame, text="Left Axis Y (File 1):", foreground="blue").pack(anchor='w', pady=(5, 0))
        self.y1_combo = ttk.Combobox(self.y_dual_frame, state='readonly')
        self.y1_combo.pack(fill='x', pady=2)
        ttk.Label(self.y_dual_frame, text="Right Axis Y (File 2):", foreground="red").pack(anchor='w', pady=(5, 0))
        ...
        # --- CONFIGURATION BUTTONS (CLEAN UI) ---
    ...
    def open_ranges_dialog(self):
        ...
        ttk.Label(frame, text="Data Transformation (Divide by)", font='bold').pack(pady=5)
        ...
        ttk.Label(frame, text="Axis Ranges", font='bold').pack(pady=5)
    ...
    def open_labels_dialog(self):
        ...
        # Increased size to prevent hiding buttons
        d.geometry("450x750")
        d.transient(self.root)
        ...
        ttk.Label(frame, text="Text Content", font='bold').pack(pady=5)
        ...
        add_entry("Z Label (Color):", self.v_zlabel)  # New Z label input
        ...
        add_col("Z Label Color:", "zlabel_color", self.zlabel_color)  # New Z Color
    ...
    def open_ticks_dialog(self):
        ...
        ttk.Label(frame, text="Tick Control", font='bold').pack(pady=5)

        # Grid header
        gf = ttk.Frame(frame);
        gf.pack(fill='x')
        ttk.Label(gf, text="Axis", width=8, font='bold').grid(row=0, column=0)
        ttk.Label(gf, text="Maj Step", width=10, font='bold').grid(row=0, column=1)
        ttk.Label(gf, text="Min Divs", width=10, font='bold').grid(row=0, column=2)
        ttk.Label(gf, text="Pad", width=10, font='bold').grid(row=0, column=3)
        ...
        ttk.Separator(frame, orient='horizontal').pack(fill='x', pady=10)
        ttk.Label(frame, text="Font Settings", font='bold').pack(pady=5)
    ...
    def populate_column_selectors(self, event):
        ...
        if self.merge_cols_var.get():
            ...
            if self.datasets:
                self.current_dataset_key = list(self.datasets.keys())[0]
        else:
        ...
        if columns:
            # Set default Y Combos for Dual Plot
            ...
            # Set default X (First column)
            if self.x_combo.get() not in columns:
                self.x_combo.set(columns[0])

            # Set default Y Listbox selection (Second column or first)
    ...
    def open_style_dialog(self):
        ...
        pairs_to_style = []
        if ptype == "Dual Y-Axis":
            y1 = self.y1_combo.get()
            y2 = self.y2_combo.get()
            if len(sel_ds) > 1:
        ...
        for fk, yc in pairs_to_style:
            ...
            def_c = '#d3d3d3'
            if ptype == "Dual Y-Axis":
                if yc == self.y1_combo.get():
                    def_c = '#1f77b4'
                elif yc == self.y2_combo.get():
                    def_c = '#d62728'

            c = st.get('color', def_c)
            w = st.get('width', 2.0)
            ls = st.get('linestyle', '-')
    ...
    def update_plot(self):
        ...
        # Standard Y uses 'y1' logic in grid if 'y' var not explicit
        # Let's map 'y' to 'y1' inputs for standard plots
        ...
        # Ranges
        z_min_val = val(self.v_z_min)  # NEW Z
        z_max_val = val(self.v_z_max)  # NEW Z
        ...
        try:
            ...
            if self.use_ref_x_var.get():
                ...
                if ref in self.datasets:
                    if xcol in self.datasets[ref].columns:
            ...
            # PLOTTING
            if ptype in ["Line", "Scatter", "Broken Y-Axis"]:
                for fk, df in sel_ds:
                    for yc in ycols:
                ...
                im = self.ax.imshow(zi, extent=(X.min(), X.max(), Y.min(), Y.max()), origin='lower', aspect='auto',
                                    cmap='seismic', vmin=z_min_val, vmax=z_max_val)  # NEW: Use Z range

                # --- FIX: Y-AXIS TITLE FOR COLOR MAP ---
                self.ax.set_ylabel(self.v_ylabel.get() or ycols[0], fontsize=l_sz, labelpad=y_lab_pad, fontname=font,
                                   color=self.ylabel_color)

                # --- FIX: Z-AXIS TITLE (COLORBAR) ---
                cbar = self.fig.colorbar(im, ax=self.ax)
                z_title = self.v_zlabel.get() or self.z_combo.get()
                cbar.set_label(z_title, fontsize=l_sz, labelpad=z_lab_pad, fontname=font, color=self.zlabel_color)
                ...
                y1n, y2n = ycols[0], ycols[1]

                self.ax.set_ylabel(self.v_ylabel.get() or y1n, fontsize=l_sz, labelpad=y_lab_pad, fontname=font,
                                   color=self.ylabel_color)
                ax_right.set_ylabel(self.v_y2label.get() or y2n, fontsize=l_sz, labelpad=y_lab_pad, fontname=font)

                # --- PRE-APPLY SPECIFIC TICKS FOR DUAL ---
                # Left (Y1)
                ax_left.tick_params(axis='y', labelsize=yt_sz, pad=y1_pad_val)
                if y1_maj: ax_left.yaxis.set_major_locator(ticker.MultipleLocator(y1_maj))
                if y1_min > 1: ax_left.yaxis.set_minor_locator(ticker.AutoMinorLocator(y1_min))

                # Right (Y2)
                ax_right.tick_params(axis='y', labelsize=yt_sz, pad=y2_pad_val)
                if y2_maj: ax_right.yaxis.set_major_locator(ticker.MultipleLocator(y2_maj))
                if y2_min > 1: ax_right.yaxis.set_minor_locator(ticker.AutoMinorLocator(y2_min))

                # 1-to-1 Mapping logic
                if len(sel_ds) > 1:
                    # File 1 -> Left
                    fk1, df1 = sel_ds[0]
                    if y1n in df1.columns:
                        if X_master is not None:
                            X1 = X_master[:min(len(X_master), len(df1))]
                            Y1 = (df1[y1n].values / yf)[:len(X1)]
                        elif xcol in df1.columns:
                            X1 = df1[xcol].values / xf
                            Y1 = df1[y1n].values / yf
                        else:
                            Y1 = None

                        if Y1 is not None:
                            sk = (fk1, y1n)
                            c = self.styles.get(sk, {}).get('color', 'tab:blue')
                            w = self.styles.get(sk, {}).get('width', 2.0)
                            ls = self.styles.get(sk, {}).get('linestyle', '-')
                            l = cust_legs[0] if len(cust_legs) > 0 else f"{fk1}: {y1n}"
                            ln, = ax_left.plot(X1, Y1, color=c, label=l, linewidth=w, linestyle=ls)
                            lines.append(ln);
                            labels.append(l)
                            ax_left.tick_params(axis='y', labelcolor=c)

                    # File 2 -> Right
                    fk2, df2 = sel_ds[1]
                    if y2n in df2.columns:
                        if X_master is not None:
                            X2 = X_master[:min(len(X_master), len(df2))]
                            Y2 = (df2[y2n].values / y2f)[:len(X2)]
                        elif xcol in df2.columns:
                            X2 = df2[xcol].values / xf
                            Y2 = df2[y2n].values / y2f
                        else:
                            Y2 = None

                        if Y2 is not None:
                            sk = (fk2, y2n)
                            c = self.styles.get(sk, {}).get('color', 'tab:red')
                            w = self.styles.get(sk, {}).get('width', 2.0)
                            ls = self.styles.get(sk, {}).get('linestyle', '--')
                            l = cust_legs[1] if len(cust_legs) > 1 else f"{fk2}:{y2n}"
                            ln, = ax_right.plot(X2, Y2, color=c, label=l, linewidth=w, linestyle=ls)
                            lines.append(ln);
                            labels.append(l)
                            ax_right.tick_params(axis='y', labelcolor=c)
                else:
                    # Single file fallback
                    fk, df = sel_ds[0]
                    if X_master is not None:
                        X = X_master[:min(len(X_master), len(df))]
                    elif xcol in df.columns:
                        X = df[xcol].values / xf
                    else:
                        X = None
                    ...
                    if X is not None:
                        if y1n in df.columns:
                            Y = (df[y1n].values / yf)[:len(X)]
                            sk = (fk, y1n)
                            c = self.styles.get(sk, {}).get('color', 'tab:blue')
                            ln, = ax_left.plot(X, Y, color=c, linewidth=2)
                            ax_left.tick_params(axis='y', labelcolor=c)
                        if y2n in df.columns:
                            Y2 = (df[y2n].values / y2f)[:len(X)]
                            sk = (fk, y2n)
                            c = self.styles.get(sk, {}).get('color', 'tab:red')
                            ln, = ax_right.plot(X, Y2, color=c, linewidth=2)
                            ax_right.tick_params(axis='y', labelcolor=c)

                # Ranges Dual
                if val(self.v_y2_min): ax_right.set_ylim(bottom=val(self.v_y2_min))
                if val(self.v_y2_max): ax_right.set_ylim(top=val(self.v_y2_max))
                if val(self.v_y_min): ax_left.set_ylim(bottom=val(self.v_y_min))
                if val(self.v_y_max): ax_left.set_ylim(top=val(self.v_y_max))

                axes_list = [ax_left, ax_right]

            # --- POST FORMATTING ---
            def apply_format(ax_obj, char, mode):
                axis = ax_obj.xaxis if char == 'x' else ax_obj.yaxis
                if mode == "Scientific":
            ...
            for ax_curr in axes_list:
                ...
                # Y settings (skip formatting for Dual Y or ColorMap as they have specific needs)
                if ptype != "Dual Y-Axis":
            ...
            # Dual Axis specific Y formatting (Notation)
            if ptype == "Dual Y-Axis":
                if not self.y_log.get():
                    apply_format(ax_left, 'y', self.v_y_not.get())
                    apply_format(ax_right, 'y', self.v_y_not.get())

            target_ax = ax2 if ptype == "Broken Y-Axis" else self.ax
            if self.x_log.get() and ptype != "Color Map":
                target_ax.set_xscale('log')
                if ptype == "Broken Y-Axis": ax1.set_xscale('log')

            target_ax.set_xlabel(self.v_xlabel.get() or xcol, fontsize=l_sz, labelpad=x_lab_pad, fontname=font,
                                 color=self.xlabel_color)
            top_ax = ax1 if ptype == "Broken Y-Axis" else self.ax
            top_ax.set_title(self.v_title.get(), fontsize=t_sz, fontweight='bold', fontname=font,
            ...
            if ptype == "Broken Y-Axis": ax1.set_xlim(target_ax.get_xlim())

            # Broken Axis Visuals
            if ptype == "Broken Y-Axis":
                b_s = val(self.v_break_start)
                b_e = val(self.v_break_end)
                if b_s and b_e:
                    ax1.set_ylim(bottom=b_e)
                    ax2.set_ylim(top=b_s)

                if val(self.v_y_max): ax1.set_ylim(top=val(self.v_y_max))
                if val(self.v_y_min): ax2.set_ylim(bottom=val(self.v_y_min))
                ...
                ax2.set_ylabel(self.v_ylabel.get() or "Values", fontsize=l_sz, fontname=font, color=self.ylabel_color)
                ax2.yaxis.set_label_coords(y_brk_off, 1.1)
            ...
            if self.show_legend.get() and ptype != "Color Map":
                if self.show_grid.get():  # Check for Grid
                    for a in axes_list: a.grid(True, alpha=0.3)
                top_ax.legend(lines, labels, loc='best', prop={'size': leg_sz, 'family': font})
    ...
    def reset_ranges(self):
        for v in [self.v_x_min, self.v_x_max, self.v_y_min, self.v_y_max, self.v_y2_min, self.v_y2_max,
                  self.v_z_min, self.v_z_max,  # Added Z here
                  self.v_break_start, self.v_break_end]:
            v.set("")
        self.update_plot()
