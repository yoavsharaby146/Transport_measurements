...
from Instruments.keithley2600 import Keithley2600, FETResultTable
from Instruments.keithley2600.keithley_driver import KeithleyIOError
import numpy as np

# local imports
from Instruments.keithleygui.pyqt_labutils import LedIndicator, SettingsWidget, ConnectionDialog
from Instruments.keithleygui.pyqtplot_canvas import SweepDataPlot
...
class SMUSettingsWidget(SettingsWidget):
    ...
    def __init__(self, smu_name):
        ...
        self.limit_i = self.addDoubleField("Current limit:", 1e-7, "A", limits=[0, 100])
        self.limit_v = self.addDoubleField("Voltage limit:", 40, "V", limits=[0, 40])
...
class OutputSweepSettingsWidget(SettingsWidget):
    def __init__(self):
        ...
        self.vd_start = self.addDoubleField("Bias sp1:", 0, "V")
        self.vd_stop = self.addDoubleField("Bias sp2:", 0, "V")
        self.vd_step = self.addDoubleField("Bias step size:", 0, "V")
...
class IVSweepSettingsWidget(SettingsWidget):
    def __init__(self, keithley):
        ...
        self.v_start = self.addDoubleField("Bias sp1:", 0, "V")
        self.v_stop = self.addDoubleField("Bias sp2:", 0, "V")
        self.v_step = self.addDoubleField("Bias step size:", 0, "V")
    ...
    def run(self):
        ...
        try:
            ...
            if self.params["sweep_type"] == "transfer":
                ...
                direction1 = np.sign(self.params["VStart"] - 0)
                stp_1 = direction1 * abs(self.params["VStep"])
                direction2 = np.sign(self.params["VStop"] - self.params["VStart"])
                stp_2 = direction2 * abs(self.params["VStep"])
                direction3 = np.sign(0 - self.params["VStop"] )
                stp_3 = direction3 * abs(self.params["VStep"])

                list_1 = np.arange(
                    0, self.params["VStart"]+ stp_1, stp_1
                )
                list_2 = np.arange(
                     self.params["VStart"], self.params["VStop"] + stp_2, stp_2
                )
                list_3 = np.arange(
                    self.params["VStop"], 0  + stp_3, stp_3
                )
                sweeplist = np.concatenate([list_1, list_2, list_3])
...
def run():
    ...
    import sys
    import argparse
    from Instruments.keithley2600 import log_to_screen
