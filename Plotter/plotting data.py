...
class InteractivePlotter:
    ...
    def setup_ui(self):
        ...
        ttk.Button(btn_frame, text="Ticks & Fonts", command=self.open_ticks_dialog).grid(row=1, column=1, padx=2,
                                                                                         pady=2, sticky='ew')
        ttk.Button(btn_frame, text="🌈 Palette Ref", command=self.show_colormap_reference).grid(row=2, column=0,
                                                                                               columnspan=2, padx=2,
                                                                                               pady=2, sticky='ew')
    ...
    def open_style_dialog(self):
        ...
        pairs_to_style = []
        # (Logic to determine which pairs to style stays the same...)
        if ptype == "Dual Y-Axis":
            y1, y2 = self.y1_combo.get(), self.y2_combo.get()
            if len(sel_ds) > 1:
                if y1: pairs_to_style.append((sel_ds[0][0], y1))
        ...
        # Table Headers
        headers = ["Series", "Color", "Width", "Type", "Action"]
        for i, h in enumerate(headers):
            ttk.Label(fr, text=h, font=('Arial', 10, 'bold')).grid(row=0, column=i, padx=5, pady=5)

        # Get the standard Matplotlib color cycle to find the defaults
        cycle = plt.rcParams['axes.prop_cycle'].by_key()['color']

        for r_idx, (fk, yc) in enumerate(pairs_to_style):
            k = (fk, yc)
            st = self.styles.get(k, {})

            # --- DEFINE DEFAULT COLOR HERE ---
            # This matches the logic in update_plot()
            def_c = cycle[r_idx % len(cycle)]

            # Current color is either what's in styles, or the default
            curr_c = st.get('color', def_c)

            ttk.Label(fr, text=f"{fk}\n{yc}").grid(row=r_idx + 1, column=0, padx=5, sticky='w')

            # Color Picker Button
            btn = tk.Button(fr, text=" ", bg=curr_c, width=5)

            def pick_c(b=btn, key=k):
                new_c = colorchooser.askcolor(parent=d)[1]
                if new_c:
                    if key not in self.styles: self.styles[key] = {}
                    self.styles[key]['color'] = new_c
                    b.config(bg=new_c)

            btn.config(command=pick_c)
            btn.grid(row=r_idx + 1, column=1)

            # Width and LineStyle entries...
            # (Truncated for brevity, keep your existing width/linestyle code here)

            # --- THE RESET BUTTON ---
            def reset_this_row(b=btn, key=k, default=def_c):
                if key in self.styles and 'color' in self.styles[key]:
                    # Remove the custom color so update_plot falls back to default
                    del self.styles[key]['color']
                b.config(bg=default)

            reset_btn = ttk.Button(fr, text="Reset", width=8,
                                   command=lambda b=btn, key=k, dc=def_c: reset_this_row(b, key, dc))
            reset_btn.grid(row=r_idx + 1, column=4, padx=5)

        ttk.Button(d, text="Apply & Close", command=lambda: [self.update_plot(), d.destroy()]).pack(pady=10)

    def show_colormap_reference(self):
        # Create a new window
        ref_window = tk.Toplevel(self.root)
        ref_window.title("Colormap Reference")
        ref_window.geometry("600x800")

        # Create a canvas with a scrollbar because the list is long
        ...
        # We only show a selection of the most popular ones to keep it readable
        # or you can use your full self.available_cmaps list
        preview_list = [
            'viridis', 'plasma', 'inferno', 'magma', 'cividis',
            'Greys', 'Purples', 'Blues', 'Greens', 'Oranges', 'Reds',
            'YlOrBr', 'YlOrRd', 'OrRd', 'PuRd', 'RdPu', 'BuPu',
        ...
        for i, name in enumerate(preview_list):
            ttk.Label(scroll_frame, text=name, font=('Arial', 10, 'bold')).pack(pady=(10, 0))

            # Create a small figure for each colormap
            fig = Figure(figsize=(5, 0.4), dpi=80)
            ax = fig.add_axes([0, 0, 1, 1])
            gradient = np.linspace(0, 1, 256).reshape(1, -1)
            ax.imshow(gradient, aspect='auto', cmap=plt.get_cmap(name))
            ax.set_axis_off()

            canvas_img = FigureCanvasTkAgg(fig, master=scroll_frame)
            canvas_img.draw()
            canvas_img.get_tk_widget().pack(padx=10, pady=2)
    ...
    def update_plot(self):
        ...
        try:
            ...
            if ptype in ["Line", "Scatter", "Broken Y-Axis", "Dual Y-Axis"]:
                ...
                #im = self.ax.imshow(zi, extent=(X.min(), X.max(), Y.min(), Y.max()), origin='lower', aspect='auto',cmap='seismic', vmin=z_min_val, vmax=z_max_val)
                im = self.ax.imshow(zi, extent=(X.min(), X.max(), Y.min(), Y.max()),
                                    origin='lower', aspect='auto',
                                    cmap=self.v_cmap_name.get())
