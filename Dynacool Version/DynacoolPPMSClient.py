...
class Cryostat:
    ...
    @property
    def field_rate(self):
        '''Current field ramp rate in Oe/s (Dynacool has a single rate, not per-range).'''
        return self._field_rate

    @field_rate.setter
    def field_rate(self, rate_Oe_per_sec):
        self._field_rate = rate_Oe_per_sec

    def go_to_target_field(self, target_field_T, rate_T_per_min=None):
        '''
        High-level sweep helper used by the lab procedures.

        Accepts target field in Tesla and an optional ramp rate in T/min
        (matching the procedures' sweep_rate parameter), converts to Oe/s
        internally, and issues the setpoint. PPMS firmware manages ramp
        safety — no per-range current limits like the Cryomagnetics supply.
        '''
        rate_Oe_per_sec = None
        if rate_T_per_min is not None:
            rate_Oe_per_sec = rate_T_per_min * self.OE_PER_TESLA / 60.0
        self.setUserMagneticField(target_field_T, rate_Oe_per_sec)

    def pause_field(self):
        '''
        Hold the field at its current value by re-issuing it as the setpoint.
        The PPMS has no separate "pause" command; this keeps the magnet at the
        present field instead of continuing toward a prior target.
        '''
        self.setUserMagneticField(self.getMagneticField())
