...
- Dynacool PPMS (temperature + magnetic field via MultiPyVu), VISA resources
  (for K2450/K2604B/SRS860/SRS830), and Zurich MFLI host/port/dev.
...
class ConfigDialog(QtWidgets.QDialog):
    ...
    def _apply_cfg_to_widgets(self) -> None:
        # PPMS (temperature + magnetic field over MultiPyVu)
    ...
    # ---------- Tabs ----------
    def _build_ppms_tab(self) -> QtWidgets.QWidget:
        w = QtWidgets.QWidget(self)
        form = QtWidgets.QFormLayout(w)

        self.chk_magnet = QtWidgets.QCheckBox("Use Dynacool PPMS (MultiPyVu)")
        self.edt_ppms_host = QtWidgets.QLineEdit()
        self.edt_ppms_host.setPlaceholderText("PPMS control PC IP, e.g. 10.0.0.10")
        self.spn_ppms_port = QtWidgets.QSpinBox()
        self.spn_ppms_port.setRange(1, 65535)
        self.spn_ppms_port.setValue(5000)

        form.addRow(self.chk_magnet)
        form.addRow("Host/IP:", self.edt_ppms_host)
        form.addRow("Port:", self.spn_ppms_port)
        return w
    ...
    def accept(self) -> None:
        ...
        cfg = InstrumentConfig(
            use_ppms=self.chk_magnet.isChecked(),
            ppms_host=self.edt_ppms_host.text().strip(),
            ppms_port=int(self.spn_ppms_port.value()),
