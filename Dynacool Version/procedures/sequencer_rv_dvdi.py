...
class RV_dV_dI_sequencer_measurement(DynacoolProcedure):
    ...
    aux_Target = FloatParameter('Auxiliary DC Bias Target (V)', group_by=['Type'],
                                group_condition='dV_dI', default=0)
    aux_signal = IntegerParameter('Auxiliary DC Signal ', group_by=['Type'],
                                  group_condition='dV_dI', default=0)
    aux_select = IntegerParameter("Auxiliary DC Select ", group_by=['Type'],
                                  group_condition='dV_dI', default=-1)
    aux_demod = IntegerParameter("Auxiliary DC demode", group_by=['Type', 'aux_select'],
                                 group_condition=['dV_dI', True, lambda v: v == 11 or v == 13])
    aux_step = FloatParameter('Auxiliary step (mV)', group_by=['Type'],
    ...
    smu = ListParameter('User defined SMU', choices=['Gate_1', 'Gate_2', 'Gate_3'],
    ...
    # NOTE: dV_dI mode requires MFLI_1 which is disabled in default config; only RV mode usable.
    ...
    def getmeas(self, t0):
        ...
        # Mid column: MFLI_1 AUX value (nan in RV mode or when MFLI off)
        if self.Type == 'dV_dI' and self.use_MFLI_1 and base._is_connected(MFLI_1):
            aux = MFLI_1.get_auxout(self.aux_signal)
        else:
            aux = math.nan
...
proc_RV_dV_dI_sequencer = {
    "RV dV_dI sequencer measurement": dict(
        ...
        inputs=['Title', 'Resistor', 'Contacts', 'Gate_contacts', 'acq_delay', 'Type', 'scan_mode',
                'Target_voltage', 'step_size', 'smu', 'devices', 'use_magnet',                 'aux_Target', 'aux_step', 'aux_signal', 'aux_select', 'aux_demod',
