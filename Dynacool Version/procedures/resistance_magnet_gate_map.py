...
class Resistance_magnet_and_gate_mapping_measurement(DynacoolProcedure):
    # --- Parameters ---
    ...
    smu = ListParameter('User defined SMU', default='Gate_1', choices=['Gate_1', 'Gate_2', 'Gate_3'],
                        group_by='mapping', group_condition=True)
    ...
    def execute(self):
        ...
        # 2. Initial Ramping (Start positions)
        # No persistent-switch heater on the Dynacool PPMS.
        ...
        while abs(magnet.getMagneticField() - self.field_start) > 0.003:
            if self.should_stop():
                magnet.pause_field()
        ...
        for i, field in enumerate(field_range):
            ...
            if i > 0:
                ...
                while abs(magnet.getMagneticField() - field) > 0.003:
                    if self.should_stop():
                        magnet.pause_field()
