"""
Base module for Dynacool procedures.

Contains common imports, instrument bindings, helper functions,
dynamic column generation, and the DynacoolProcedure base class with
centralized instrument reading that skips disconnected instruments.

Supported instruments:
  - Magnet (Dynacool PPMS via MultiPyVu)
  - 3x Keithley 2450 (Gate_1, Gate_2, Gate_3)
  - 2x SRS SR860 (SRS860_1, SRS860_2)
  - 3x SRS SR830 (SRS830_1, SRS830_2, SRS830_3)
  - 3x Zurich MFLI (MFLI_1, MFLI_2, MFLI_3)

No Dual_gate (Keithley 2604B) in this setup.
"""
...
def _build_smu_columns():
    """SMU columns for connected Keithley 2450 gates only."""
    cols = []
...
def _build_base_columns():
    ...
    return ['time(s)', 'sample_temp(K)'] + _build_smu_columns()


def _rebuild_procedure_columns(cls):
    """Rebuild DATA_COLUMNS for a single DynacoolProcedure subclass."""
...
def _rebuild_all_columns():
    """Rebuild module-level column lists and all DynacoolProcedure DATA_COLUMNS.
    ...
    # Rebuild DATA_COLUMNS on every DynacoolProcedure subclass found in procedure modules
    for mod in sys.modules.values():
        ...
        for attr_name in list(mod.__dict__):
            ...
            if (isinstance(obj, type) and issubclass(obj, DynacoolProcedure)
                    and obj is not DynacoolProcedure):
...
# ---------------- DynacoolProcedure base class ----------------

class DynacoolProcedure(Procedure):
    """Base class for Dynacool procedures.
    ...
    def _read_smu_values(self):
        """Read connected Keithley 2450 gates. NaN for connected-but-disabled, skip disconnected."""
        vals = []
        if _is_connected(Gate_1):
    ...
    def _read_magnet(self):
        ...
        if not _is_connected(magnet):
            return []
        return [magnet.getMagneticField() if self.use_magnet else math.nan]
...
def _rebind_instruments_from_configuration():
    ...
    global magnet, MFLI_1, MFLI_2, MFLI_3, SRS860_1, SRS860_2, SRS830_1, SRS830_2, SRS830_3, Gate_1, Gate_2, Gate_3
    magnet = _cfg.magnet
    MFLI_1 = _cfg.MFLI_1
    MFLI_2 = _cfg.MFLI_2
    MFLI_3 = _cfg.MFLI_3
