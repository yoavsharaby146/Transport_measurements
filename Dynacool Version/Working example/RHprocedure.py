...
##                    break
    ...
    def execute(self):
        ...
        current_field = self.ppms.getMagneticField()
##        current_field = 20
        time.sleep(0.3)
        try:
            self.ppms.setUserMagneticField(self.field_setpoint)
        except:
            print("failed setting magnetic field")
            time.sleep(10)
        if self.ppms.isControllingField() == 0:
            time.sleep(0.5)
            self.ppms.toggleMagneticFieldControl()
        ...
        while (abs(current_field - self.field_setpoint) > 0.002):
            ...
            if key_pressed() == b'\x1b':
                ...
                self.ppms.setUserMagneticField(self.ppms.getMagneticField())
                break
...
def main(ppms, folder_path, field_setpoint = 0):
    global new_logging
