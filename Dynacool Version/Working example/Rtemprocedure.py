...
class RTProcedure(Procedure):
    ...
    def startup(self):
        ...
        while self.ppms.getSampleTemperature() == 0:
            time.sleep(0.5)
            print(".",end='')
##        self.ppms.toggleFullTemperatureControl()
##        time.sleep(2)
##        self.ppms.toggleFullTemperatureControl()
##        log.info("Temperature control initialized")
    ...
    def execute(self):
        ...
        while self.ppms.getSampleTemperature() == 0:
            time.sleep(0.5)
            print(".",end='')
        current_temperature = self.ppms.getSampleTemperature()
        # make sure that the temperature toggle is on
        time.sleep(1)
        ToggleFlag = self.ppms.isControllingTemperature()
        if ToggleFlag == 0:
##            print(ToggleFlag)
            time.sleep(1)
            self.ppms.toggleFullTemperatureControl()
...
##        while abs(deltaT) > Taccuracy:
        ...
        for temp in temps:
            self.ppms.setUserTemperature(temp)
            current_temperature = self.ppms.getSampleTemperature()
...
def main(ppms, folder_path, temperature_setpoint, temperature_delay, temperature_step):
    global new_logging
