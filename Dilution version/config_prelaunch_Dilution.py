...
class InstrumentConfig:
    # dilution refrigerator network interface
    use_dilution: bool = False
    dilution_ip: str = ""
    dilution_port: int = 33576
...
class ConfigDialog(QtWidgets.QDialog):
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None):
        ...
        self._dilution_tab = self._build_dilution_tab()

        # Magnet tab removed (dilution instrument provides field readings)
        ...
        self.tabs.addTab(self._dilution_tab, "Dilution")
    ...
    def _apply_cfg_to_widgets(self) -> None:
        ...
        self.chk_dilution.setChecked(getattr(self._cfg, "use_dilution", False))
        self.edt_dilution_ip.setText(getattr(self._cfg, "dilution_ip", ""))
        self.spn_dilution_port.setValue(int(getattr(self._cfg, "dilution_port", 33576)))
        # Gate 1/2 and Dual
    ...
    def _refresh_all(self):
        ...
        # COM port list is not used in dilution mode, but calling stub for compatibility
        self._fill_com_ports(self._com_list())


    # ---------- Tabs ----------

    def _build_dilution_tab(self) -> QtWidgets.QWidget:
        w = QtWidgets.QWidget(self)
        form = QtWidgets.QFormLayout(w)

        self.chk_dilution = QtWidgets.QCheckBox("Use Dilution Fridge")
        self.edt_dilution_ip = QtWidgets.QLineEdit()
        self.edt_dilution_ip.setPlaceholderText("e.g. 132.66.132.173")
        self.spn_dilution_port = QtWidgets.QSpinBox()
        self.spn_dilution_port.setRange(1, 65535)
        self.spn_dilution_port.setValue(33576)

        form.addRow(self.chk_dilution)
        form.addRow("IP Address:", self.edt_dilution_ip)
        form.addRow("Port:", self.spn_dilution_port)
        return w
    ...
    def _fill_com_ports(self, ports: List[str]):
        # No magnet tab in dilution config; stub for compatibility with original script.
        # If you later add a COM combobox, update this method accordingly.
        return
    ...
    def accept(self) -> None:
        ...
        cfg = InstrumentConfig(
            use_dilution=self.chk_dilution.isChecked(),
            dilution_ip=self.edt_dilution_ip.text().strip(),
            dilution_port=int(self.spn_dilution_port.value()),
